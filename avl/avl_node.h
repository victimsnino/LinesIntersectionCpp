...
Node<KeyType>* Node<KeyType>::BalanceIfNeeded()
{
    ...
    if (diff_heights == 2) // right child height > left child
    {
        ...
        if (right_node->GetDiffHeight() < 0) // his left is longer, then we need to do big left rotate (rotate right child  right and then left self)
...
Node<KeyType>* Node<KeyType>::RotateLeft()
{
    ...
    BaseNode<KeyType>::GetRightNode() = right_child->GetLeftNode();
...
Node<KeyType>* Node<KeyType>::RotateRight()
{
    ...
    BaseNode<KeyType>::GetLeftNode() = left_child->GetRightNode();
